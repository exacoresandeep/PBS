...
<div class="activity-sec">
    ...
    <div class="listing-sec">
        <table class="table table-bordered table-striped w-100 table-responsive"  style="display: table !important;" id="activityTypeTable">
            <thead>
                <tr>
                    <th>Sl.No</th>
                    <th>Activity Name</th>
...
<script>
    $(document).ready(function () {
        ...
        $('#activityTypeForm').submit(function (e) {
            ...
            let activity_type_id = $('#activity_type_id').val();
            let url = activity_type_id 
                ? "{{ route('sales.activity.type.update', ':id') }}".replace(':id', activity_type_id) 
                : "{{ route('sales.activity.type.store') }}";
            
            let method = activity_type_id ? 'PUT' : 'POST';
