        ...
        try {
         ...
         //   $validatedData['billing_date'] = Carbon::createFromFormat('d/m/Y', $validatedData['billing_date'])->format('Y-m-d');
           ...
           // if (!empty($validatedData['delivery_date'])) {
             //   $validatedData['delivery_date'] = Carbon::createFromFormat('d/m/Y', $validatedData['deliver
	    //
	    //
	    $validatedData['billing_date'] = $this->normalizeDate($validatedData['billing_date']);
        $validatedData['delivery_date'] = $this->normalizeDate($validatedData['delivery_date'] ?? null);
...
'delivery_date' => $order->delivery_date
	...
	$productId = $request->input('product_id'); // ✅ NEW
		    ...
		    'data' => [
			    "dealers"=>$dealerRouteIds,
                        "dasealers"=>$assignedRouteIds,
		    ]
                ], 403);
            }
    ...
    public function monthlyTargetAchievement(Request $request)
    {
		...
		$productId = $request->input('product_id'); 
		 ...
		 $monthName = Carbon::createFromDate(null, $month, 1)->format('F');
            $target = DealerTarget::where('dealer_id', $dealer->id)
		    ...
		    "mname" => $dealer->id,
                    ...
                    'target_quantity' => $target->order_quantity ?? 0,
...
public function paymentHistoryList(Request $request)
    ...
    public function paymentHistoryOrderDetails(Request $request, $orderId)
    {
		...
		'invoice_date' => $invoiceDate,
                ...
                'billing_date' =>  $order->billing_date ?? null,//optional($order->billing_date)->format('d/m/Y'),
