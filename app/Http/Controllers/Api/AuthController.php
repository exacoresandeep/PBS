		...
		$employee->tokens()->delete();
         ...
         return response()->json([
			    ...
			    'name' => $employee->name,
                   ...
                   //         'designation' => $employee->designation,
                     'designation' => $employee->type_name == "Area Sales Manager" ? "District Sales Manager" : $employee->type_name,
                           'email' => $employee->email,
        ...
        try {
            ...
            if (in_array($productCode, ['durashine', 'structura'])) {
                ...
                $query = ProductDetails::select(
                    ...
                    ->where('products_details.availability_status', 'Available')->where('products_details.status', 'Active');
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'tonnage_per_piece' => $tonnage !== null 
                                ? (float) number_format($tonnage, 4, '.', '') 
                                : null,
                ]
            ]);
