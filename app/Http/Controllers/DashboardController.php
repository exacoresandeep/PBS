    ...
    public function getLeadsData(Request $request)
{
        ...
        ->where(function ($q) use ($year, $month) {
            ...
            ->orWhere(function ($q2) use ($year, $month) {
                $q2->whereYear('follow_up_date', $year)
                   ->whereMonth('follow_up_date', $month + 1);
            });
    ...
    public function getLeadsDataOld(Request $request)
    {
        $month = $request->month; 
        $year = $request->year;
        $productID= \App\Helpers\ProductHelper::getSelectedProductID();
    ...
    public function getOverallTargetAndAchievement(Request $request)
    {
        ...
        //                     });
        $customerVisitCount = InfluencerVisit::where(function ($q) use ($year, $monthNumber) {

                $q->where(function ($q1) use ($year, $monthNumber) {
                    $q1->whereYear('updated_at', $year)
                    ->whereMonth('updated_at', $monthNumber+1);
                })

                ->orWhere(function ($q2) use ($year, $monthNumber) {
                    $q2->whereYear('follow_up_date', $year)
    ...
    public function getInfluencerVisitDataOld(Request $request)
{
    ...
    $totalInfluencerVisit = \App\Models\InfluencerVisit::whereHas('createdBy', function ($q) use ($productID) {
            $q->whereJsonContains('products', (string) $productID);
        })
