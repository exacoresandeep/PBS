            ...
            if ($response->successful() && strtolower($responseBody) === 'success') {
                ...
                try {
                    ...
                    if ($deviceToken) {
                        ...
                        $body = 'Order No. OD00' . $order->id . ' approved successfully';
...
$deliveryDate = Carbon::createFromFormat('d/m/Y', $order->delivery_date);
        ...
        $sapPayload = [
            ...
            'ShipTo'        => $order->dealer->dealer_name,
            'SO_No'      => $order->id,
            'SO_Date'       => $today->format('Y-m-d'),// $order->created_at->format('Y-m-d'),
            'Delivery_Date' => $deliveryDate->format('Y-m-d'),//optional($order->delivery_date)->format('Y-m-d') ?? now()->addDays(7)->format('Y-m-d'),
...
//	if ($response->successful() && strtolower($responseBody) === 'success') {
                ...
                try {
                    ...
                    if ($deviceToken) {
                        $title = 'Prabhu Steels Sale App Notification';
                        $body = 'Order No. OD00' . $order->id . ' approved successfully';
                        $fcm->sendNotification($deviceToken, $title, $body,  $sender);
                    }
                } catch (\Exception $e) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'SAP Error: ' . $responseData,
                ], 500);
            }
        } catch (\Exception $e) {
            return response()->json([
	...
	$order->save();
	 ...
	 // Send FCM notification to assigned Dealer
        try {
            ...
            if ($deviceToken) {
                $title = 'Prabhu Steels Sales App Notification';
                $body = 'Order No. OD00' . $order->id . ' is rejected';
                $fcm->sendNotification($deviceToken, $title, $body,  $sender);
            }
        } catch (\Exception $e) {
        }
